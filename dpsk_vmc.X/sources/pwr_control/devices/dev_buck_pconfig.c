...
#include "templates/dev_buck_ptemp_dac.h"
...
volatile uint16_t buckAuxPWM_ChannelInitialize(volatile struct BUCK_POWER_CONTROLLER_s* buckInstance)
{
    volatile uint16_t retval=1;
    volatile uint16_t _i=0;
    ...
    for (_i=0; _i<buckInstance->set_values.no_of_phases; _i++) {
        ...
        // LOAD PERIPHERAL INSTANCES FROM BUCK CONVERTER OBJECT
        pwm_Instance = buckInstance->aux_pwm.pwm_instance;
        gpio_Instance = buckInstance->aux_pwm.gpio_instance;

        // CAPTURE MEMORY ADDRESS OF GIVEN PWM GENERATOR INSTANCE
        gpio = p33c_GpioInstance_GetHandle(gpio_Instance);

        // WRITE GPIO CONFIGURATION OF PWM OUTPUT(S)
        gpio->LATx.value  &= ~(0x0001 << buckInstance->aux_pwm.gpio_high); // Clear PWMxH output LOW
        gpio->LATx.value  &= ~(0x0001 << buckInstance->aux_pwm.gpio_low);  // Clear PWMxL output LOW
        gpio->TRISx.value &= ~(0x0001 << buckInstance->aux_pwm.gpio_high); // Clear PWMxH output to OUTPUT
        gpio->TRISx.value &= ~(0x0001 << buckInstance->aux_pwm.gpio_low);  // Clear PWMxL output to OUTPUT
        gpio->CNPDx.value |= (0x0001 << buckInstance->aux_pwm.gpio_high);  // Enable intern pull down register (PWM1H)
        gpio->CNPDx.value |= (0x0001 << buckInstance->aux_pwm.gpio_low);   // Enable intern pull down register (PWM1L)
    
        // COPY CONFIGURATION FROM TEMPLATE TO PWM GENERATOR x CONTROL REGISTERS
        pg = p33c_PwmGenerator_GetHandle(pwm_Instance);
        retval &= p33c_PwmGenerator_ConfigWrite(buckInstance->aux_pwm.pwm_instance, buckAuxPwmGeneratorConfig);

        // LOAD PWM GENERATOR TIMING SETTINGS FROM BUCK CONVERTER OBJECT
        pg->PGxCONH.bits.MPERSEL = (uint16_t)buckInstance->aux_pwm.master_period_enable;
        pg->PGxCONL.bits.HREN = (uint16_t)buckInstance->aux_pwm.high_resolution_enable;
        
        pg->PGxPHASE.value= buckInstance->aux_pwm.phase;
        pg->PGxDC.value = buckInstance->aux_pwm.duty_ratio_min; // PGxDC: PWM GENERATOR x DUTY CYCLE REGISTER
        pg->PGxPER.value = buckInstance->aux_pwm.period; // PGxPER: PWM GENERATOR x PERIOD REGISTER
        pg->PGxDTL.value = buckInstance->aux_pwm.dead_time_falling; // PGxDTL: PWM GENERATOR x DEAD-TIME REGISTER LOW
        pg->PGxDTH.value = buckInstance->aux_pwm.dead_time_rising; // PGxDTH: PWM GENERATOR x DEAD-TIME REGISTER HIGH
        pg->PGxLEBL.value = buckInstance->aux_pwm.leb_period; // PWM GENERATOR x LEADING-EDGE BLANKING REGISTER LOW 
        ...
        if( _i == 0) {
            ...
            pg->PGxCONH.bits.SOCS = 0b0000; // Master PWM is self-triggered
            pg->PGxEVTL.bits.PGTRGSEL = 0b011; // Master PWM uses PGxTRIGC as master trigger output
            pg->PGxTRIGC.bits.TRIG = buckInstance->aux_pwm.phase; // Set phase shift of trigger
        }
        else {
            pg->PGxCONH.bits.MSTEN = 0; // Make all other PWMs of switch node objects SLAVES
            pg->PGxCONH.bits.UPDMOD = P33C_PGxCONH_UPDMOD_SLV; // Slave PWMs update PWM registers Immediately at MASTER trigger
...
volatile uint16_t buckAuxPWM_Start(volatile struct BUCK_POWER_CONTROLLER_s* buckInstance) 
{
    volatile uint16_t retval=1;
    volatile uint16_t _i=0;
    volatile uint16_t timeout=0;
...
volatile uint16_t buckADC_ModuleInitialize(void) 
{
    ...
    ADCON3Hbits.CLKSEL = 0b10;//0b01; // ADC Module Clock Source Selection: AVCODIV
...
volatile uint16_t buckDAC_Initialize(volatile BUCK_POWER_CONTROLLER_t* buckInstance)
{
    volatile uint16_t retval=1;
    ...
    volatile struct P33C_DAC_INSTANCE_s* my_dac_instance;
     ...
     retval &=p33c_DacModule_ConfigWrite(buckDacModuleConfig);
     
     my_dac_module->DacModuleCtrl1L.value = 0b0000000010000000; //AFPLLO 
     my_dac_module->DacModuleCtrl2H.value = buckInstance->slope_comp.start_settle_delay;                          
     my_dac_module->DacModuleCtrl2L.value = buckInstance->slope_comp.transition_delay;
     
     my_dac_instance = p33c_DacInstance__GetHandle(buckInstance->slope_comp.dac_instance); //// user-definaed DAC 1 object
     retval &=p33c_DacInstance_ConfigWrite(buckInstance->slope_comp.dac_instance,buckDacInstanceConfig);
     
     my_dac_instance->DACxCONH.value = buckInstance->slope_comp.leading_edge_blanking;
    
     my_dac_instance->SLPxDAT.value = buckInstance->slope_comp.slope_rate;
     my_dac_instance->DACxDATH.value =  buckInstance->slope_comp.dac_value_high;
     my_dac_instance->DACxDATL.value = buckInstance->slope_comp.dac_value_low;
    
     my_dac_instance->DACxCONL.value |= 0b1000001000000000;    // set DACOEN and DACEN ,Output DAC voltage to DACOUT1 pin
     my_dac_module->DacModuleCtrl1L.value |= 0b1000000000000000;    // set DACON ,enable DAC module
     return (retval);
}
