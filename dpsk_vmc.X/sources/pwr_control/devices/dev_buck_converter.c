...
volatile uint16_t drv_BuckConverter_Initialize(volatile struct BUCK_POWER_CONTROLLER_s *buckInstance) 
{
    ...
    //retval &= buckAuxPWM_ChannelInitialize(buckInstance);  // Initialize PWM Channel of Buck Converter
    //retval &= buckAuxPWM_Start(buckInstance);
    retval &= buckDAC_Initialize(buckInstance);
