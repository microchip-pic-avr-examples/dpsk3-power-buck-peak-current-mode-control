...
volatile uint16_t SubState_PrepareVRampUp(volatile struct BUCK_CONVERTER_s *buckInstance)
{
    ...
    {
        // If pointer to duty cycle register is invalid, return error
        if (buckInstance->sw_node[_i].slope_compensation.ptr_duty_cycle == NULL)
        { return(BUCK_OPSRET_ERROR); }

        // Set duty cycle limit to allow comparator control
        for (_i=0; _i<buckInstance->set_values.no_of_phases; _i++) {
            *buckInstance->sw_node[_i].slope_compensation.ptr_duty_cycle = 
                buckInstance->sw_node[_i].slope_compensation.duty_cycle_limit;
        }
    }
