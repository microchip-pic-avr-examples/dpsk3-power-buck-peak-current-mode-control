...
volatile uint16_t buckPWM_ChannelInitialize(volatile struct BUCK_CONVERTER_s* buckInstance)
{
    ...
    for (_i=0; _i<buckInstance->set_values.no_of_phases; _i++) {
        ...
        // In Peak Current Mode Control, initialize PWM channel current limit PCI interface
        if(buckInstance->set_values.control_mode == BUCK_CONTROL_MODE_PCMC) {
            
            pg->PGxEVTL.bits.ADTR1EN1 = true; // Use PGxTRIGA as PWM trigger #1
    ...
    // In Peak Current Mode Control, initialize comparator and PDM DAC instances
    if(buckInstance->set_values.control_mode == BUCK_CONTROL_MODE_PCMC) {
        retval &= buckDAC_ModuleInitialize();
        retval &= buckDAC_ChannelInitialize(buckInstance);
    }    
...
/*******************************************************************************
 * @fn	    uint16_t buckDAC_ModuleInitialize(void)
 * @ingroup lib-layer-buck-pconfig-functions
 * @brief   This function initializes the Digital-To-Analog converter peripheral module
 * @return  unsigned integer (0=failure, 1=success)
 *  
 * @details
 * 
...
{
    volatile uint16_t retval=1;
    volatile struct P33C_DAC_MODULE_s* dacModule;     // user-defined DAC object 

    dacModule = p33c_DacModule_GetHandle();                    // Get pointer address to DAC module
    retval &=p33c_DacModule_ConfigWrite(buckDacModuleConfig);  // Initialize DAC module with default values

    dacModule->DacModuleCtrl1L.bits.DACON = 0;        // Make sure DAC module is turned off
    dacModule->DacModuleCtrl1L.value = REG_DACCTRL1L; // Load default template
    dacModule->DacModuleCtrl2H.value = DAC_SSTIME;    // Load default steady-state time                      
    dacModule->DacModuleCtrl2L.value = DAC_TMODTIME;  // Load default transition time
    
    return(retval);
}

/*******************************************************************************
 * @fn	    uint16_t buckDAC_ChannelInitialize(volatile struct BUCK_CONVERTER_s* buckInstance)
 * @ingroup lib-layer-buck-pconfig-functions
 * @brief   This function initializes the Digital-To-Analog converter, slope compensation and analog comparator used in peak current mode control 
 * @param	buckInstance  Pointer to a Buck Converter data object of type struct BUCK_CONVERTER_s
 * @return  unsigned integer (0=failure, 1=success)
 *  
 * @details
...
volatile uint16_t buckDAC_ChannelInitialize(volatile struct BUCK_CONVERTER_s* buckInstance)
{
    volatile uint16_t retval=1;
    volatile uint16_t _i=0;
    volatile uint16_t bit_mask=0;
    volatile struct P33C_DAC_INSTANCE_s* dacInstance; // user-defined DAC instance
    
    for(_i=0; _i<buckInstance->set_values.no_of_phases; _i++) 
    {
        dacInstance = p33c_DacInstance_GetHandle(buckInstance->sw_node[_i].slope_compensation.dac_instance); // user-defined DAC 1 object
        bit_mask = (dacInstance->DACxCONL.value & REG_DACOEN_EN); // Capture DAC Output Enable bit setting

        retval &=p33c_DacInstance_ConfigWrite(buckInstance->sw_node[_i].slope_compensation.dac_instance, buckDacInstanceConfig);

        dacInstance->DACxCONL.value = (REG_DACxCONL_PC | bit_mask); // Load PCMC default configuration, including previous DAC output setting
        dacInstance->DACxCONL.bits.INSEL = buckInstance->sw_node[_i].slope_compensation.cmp_input;
        
        dacInstance->DACxCONH.value = REG_DACxCONH; // Load PCMC default configuration
        dacInstance->DACxCONH.bits.TMCB = buckInstance->sw_node[_i].leb_period; // Load leading edge blanking value

        dacInstance->SLPxDAT.value  = buckInstance->sw_node[_i].slope_compensation.slew_rate; // Set DAC slope compensation slew rate
        dacInstance->DACxDATH.value = buckInstance->sw_node[_i].slope_compensation.initial_dac_value; // Set initial DAC reference
        dacInstance->DACxDATL.value = 0; // Clear DAC LOW register
        
        dacInstance->SLPxCONL.value = REG_SLPxCONL; // Enabling negative slope mode
...
/*******************************************************************************
 * @fn	    uint16_t buckDAC_Start(void)
 * @ingroup lib-layer-buck-pconfig-functions
 * @brief   This function initializes the Digital-To-Analog converter peripheral module
 * @return  unsigned integer (0=failure, 1=success)
 *  
 ...
 * 
 *********************************************************************************/
volatile uint16_t buckDAC_Start(volatile struct BUCK_CONVERTER_s* buckInstance)
{
    volatile uint16_t _i=0;
    volatile uint16_t retval=1;
    volatile struct P33C_DAC_MODULE_s* dacModule;     // user-defined DAC object 
    ...
    for(_i=0; _i<buckInstance->set_values.no_of_phases; _i++) 
    {
        dacInstance = p33c_DacInstance_GetHandle(buckInstance->sw_node[_i].slope_compensation.dac_instance); // Get pointer address to DAC module
        dacInstance->SLPxCONH.bits.SLOPEN = true;   // Turn on slope generator
        dacInstance->DACxCONL.bits.DACEN = true;    // Turn on DAC instance
    }

    dacModule = p33c_DacModule_GetHandle();         // Get pointer address to DAC module
    dacModule->DacModuleCtrl1L.bits.DACON = true;   // turn on DAC module
    
    return(retval);
}

/*******************************************************************************
 * @fn	    uint16_t buckDAC_Stop(void)
 * @ingroup lib-layer-buck-pconfig-functions
 * @brief   This function initializes the Digital-To-Analog converter peripheral module
 * @return  unsigned integer (0=failure, 1=success)
 *  
 ...
 * 
 *********************************************************************************/
volatile uint16_t buckDAC_Stop(volatile struct BUCK_CONVERTER_s* buckInstance)
{
    volatile uint16_t _i=0;
    volatile uint16_t retval=1;
    volatile struct P33C_DAC_INSTANCE_s* dacInstance; // user-defined DAC instance
    ...
    {
        dacInstance = p33c_DacInstance_GetHandle(buckInstance->sw_node[_i].slope_compensation.dac_instance); // Get pointer address to DAC module
        dacInstance->DACxCONL.bits.DACEN = false;       // Turn on DAC instance
    }
